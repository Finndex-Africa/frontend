'use client';

import { useState, useRef, useEffect } from 'react';
import { Share2, Link2, ChevronDown } from 'lucide-react';

interface ShareButtonProps {
    title: string;
    text: string;
    url?: string;
    className?: string;
    /** Compact: only icon, for use on cards (use with onClick stopPropagation on parent) */
    compact?: boolean;
    /** For compact button: align dropdown to the right of the button */
    dropdownRight?: boolean;
}

const FACEBOOK_SHARE = 'https://www.facebook.com/sharer/sharer.php?u=';
const TWITTER_SHARE = 'https://twitter.com/intent/tweet';
const WHATSAPP_SHARE = 'https://wa.me/?text=';
const LINKEDIN_SHARE = 'https://www.linkedin.com/sharing/share-offsite/?url=';

export default function ShareButton({ title, text, url, className, compact, dropdownRight }: ShareButtonProps) {
    const [open, setOpen] = useState(false);
    const [copied, setCopied] = useState(false);
    const dropdownRef = useRef<HTMLDivElement>(null);

    const getResolvedUrl = () => {
        const u = url || (typeof window !== 'undefined' ? window.location.href : '');
        if (typeof window !== 'undefined' && u && !u.startsWith('http')) {
            return window.location.origin + (u.startsWith('/') ? u : '/' + u);
        }
        return u;
    };
    const shareUrl = getResolvedUrl();
    const encodedUrl = typeof window !== 'undefined' ? encodeURIComponent(shareUrl) : '';
    const encodedText = typeof window !== 'undefined' ? encodeURIComponent(text) : '';

    useEffect(() => {
        const handleClickOutside = (e: MouseEvent) => {
            if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) {
                setOpen(false);
            }
        };
        document.addEventListener('mousedown', handleClickOutside);
        return () => document.removeEventListener('mousedown', handleClickOutside);
    }, []);

    const openShare = (href: string) => {
        if (typeof window !== 'undefined') {
            window.open(href, '_blank', 'noopener,noreferrer,width=600,height=500');
        }
        setOpen(false);
    };

    const handleCopyLink = async () => {
        try {
            if (typeof navigator !== 'undefined' && navigator.clipboard?.writeText) {
                await navigator.clipboard.writeText(shareUrl);
            } else {
                const textArea = document.createElement('textarea');
                textArea.value = shareUrl;
                textArea.style.position = 'fixed';
                textArea.style.opacity = '0';
                document.body.appendChild(textArea);
                textArea.select();
                document.execCommand('copy');
                document.body.removeChild(textArea);
            }
            setCopied(true);
            setTimeout(() => setCopied(false), 2000);
            setOpen(false);
        } catch {
            setOpen(false);
        }
    };

    const handleNativeShare = async () => {
        if (typeof navigator !== 'undefined' && navigator.share) {
            try {
                await navigator.share({ title, text, url: shareUrl });
                setOpen(false);
            } catch (err) {
                if ((err as Error).name !== 'AbortError') {
                    handleCopyLink();
                }
            }
        } else {
            handleCopyLink();
        }
    };

    const hasNativeShare = typeof navigator !== 'undefined' && !!navigator.share;

    return (
        <div className="relative inline-block" ref={dropdownRef}>
            <button
                type="button"
                onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    setOpen((o) => !o);
                }}
                className={className || (compact ? 'p-2 rounded-full bg-white/10 backdrop-blur-sm hover:bg-white/20 transition-colors text-white' : 'flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors')}
                title="Share"
                aria-haspopup="true"
                aria-expanded={open}
            >
                <Share2 className="w-4 h-4" />
                {!compact && (
                    <>
                        <span>Share</span>
                        <ChevronDown className={`w-4 h-4 transition-transform ${open ? 'rotate-180' : ''}`} />
                    </>
                )}
            </button>

            {open && (
                <div
                    className={`absolute top-full mt-1 z-[9999] min-w-[200px] py-1 bg-white border border-gray-200 rounded-xl shadow-lg ${dropdownRight ? 'right-0 left-auto' : 'left-0'}`}
                    role="menu"
                >
                    <p className="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-100">
                        Share via
                    </p>
                    <a
                        href={`${FACEBOOK_SHARE}${encodedUrl}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={(e) => {
                            e.preventDefault();
                            openShare(`${FACEBOOK_SHARE}${encodedUrl}`);
                        }}
                        className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-800 hover:bg-blue-50 transition-colors"
                        role="menuitem"
                    >
                        <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-[#1877F2]/10 text-[#1877F2]">
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                            </svg>
                        </span>
                        Facebook
                    </a>
                    <a
                        href={`${TWITTER_SHARE}?url=${encodedUrl}&text=${encodedText}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={(e) => {
                            e.preventDefault();
                            openShare(`${TWITTER_SHARE}?url=${encodedUrl}&text=${encodedText}`);
                        }}
                        className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-800 hover:bg-gray-100 transition-colors"
                        role="menuitem"
                    >
                        <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-black/5 text-gray-900">
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                            </svg>
                        </span>
                        X (Twitter)
                    </a>
                    <a
                        href={`${WHATSAPP_SHARE}${encodeURIComponent(`${text} ${shareUrl}`)}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={(e) => {
                            e.preventDefault();
                            openShare(`${WHATSAPP_SHARE}${encodeURIComponent(`${text} ${shareUrl}`)}`);
                        }}
                        className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-800 hover:bg-green-50 transition-colors"
                        role="menuitem"
                    >
                        <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-[#25D366]/10 text-[#25D366]">
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                            </svg>
                        </span>
                        WhatsApp
                    </a>
                    <a
                        href={`${LINKEDIN_SHARE}${encodedUrl}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={(e) => {
                            e.preventDefault();
                            openShare(`${LINKEDIN_SHARE}${encodedUrl}`);
                        }}
                        className="flex items-center gap-3 px-3 py-2.5 text-sm text-gray-800 hover:bg-blue-50 transition-colors"
                        role="menuitem"
                    >
                        <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-[#0A66C2]/10 text-[#0A66C2]">
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                            </svg>
                        </span>
                        LinkedIn
                    </a>
                    <div className="border-t border-gray-100 mt-1 pt-1">
                        <button
                            type="button"
                            onClick={handleCopyLink}
                            className="flex items-center gap-3 w-full px-3 py-2.5 text-sm text-gray-800 hover:bg-gray-50 transition-colors"
                            role="menuitem"
                        >
                            <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 text-gray-600">
                                <Link2 className="w-4 h-4" />
                            </span>
                            {copied ? 'Link copied!' : 'Copy link'}
                        </button>
                        {hasNativeShare && (
                            <button
                                type="button"
                                onClick={handleNativeShare}
                                className="flex items-center gap-3 w-full px-3 py-2.5 text-sm text-gray-800 hover:bg-gray-50 transition-colors"
                                role="menuitem"
                            >
                                <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 text-gray-600">
                                    <Share2 className="w-4 h-4" />
                                </span>
                                More options…
                            </button>
                        )}
                    </div>
                </div>
            )}
        </div>
    );
}
