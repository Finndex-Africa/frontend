...
export class AuthService {
  ...
  logout(reason: string = "manual"): void {
    logInfo("AuthService: logout invoked", {
      reason,
    ...
    if (DEBUG_PRESERVE_NETWORK) {
      console.warn(
        "[DEBUG_PRESERVE_NETWORK] Skipping full-page redirect to /routes/login (storage was cleared).",
      );
    } else {
  ...
  async refreshToken(): Promise<{ token: string; user: IUser }> {
    try {
      ...
      // If refresh fails with 401, logout (unless debugging network / redirects)
      if (error.response?.status === 401 && !DEBUG_PRESERVE_NETWORK) {
  ...
  public getUser(): IUser | null {
    ...
    try {
        ...
        try {
          ...
          if (parsed && (parsed.email || parsed.Email)) {
            ...
            const normalized: IUser = {
              ...
              role:
                (parsed.role as string) ?? (parsed.userType as string) ?? "",
            };
            return normalized;
          }
        } catch (err) {
          logDebug(
            "AuthService: failed to parse stored user, will decode token",
            { err: String(err) },
          );
      ...
      if (decodedToken && decodedToken.exp && decodedToken.exp < currentTime) {
        ...
        logInfo("AuthService: token expired - clearing storage", {
          exp: decodedToken.exp,
          expiredTime,
          now: currentTime,
        });
        console.warn(
          `[AUTH] Token expired at ${expiredTime}. Clearing storage.`,
        );
        window.localStorage.removeItem("token");
        window.localStorage.removeItem("authToken");
        window.localStorage.removeItem("user");
      ...
      if (decodedToken) {
        ...
        if (reconstructed.email || reconstructed.id) {
          logDebug("AuthService: reconstructed user from token", {
            reconstructed,
          });
      ...
      logDebug("AuthService: getUser error (not clearing token)", {
        err: String(error),
      });
    ...
    axios.interceptors.response.use(
      ...
      (error) => {
        try {
          ...
          logError("AuthService: axios response error", {
            status,
            url,
          ...
          if (status === 401) {
            try {
              ...
              if (skipLogout) {
                logInfo(
                  DEBUG_PRESERVE_NETWORK
                    ? "AuthService: 401 — skipping auto-logout (DEBUG_PRESERVE_NETWORK)"
                    : "AuthService: detected 401 response but skipping auto-logout on localhost",
                  { url },
                );
                console.warn(
                  `[AUTH] 401 Unauthorized on ${url}${
                    DEBUG_PRESERVE_NETWORK
                      ? " — auto-logout skipped (DEBUG_PRESERVE_NETWORK)"
                      : " - auto-logout skipped (localhost)"
                  }`,
                );
              } else {
                logInfo("AuthService: detected 401 response, invoking logout", {
                  url,
                });
                console.warn(
                  `[AUTH] 401 Unauthorized on ${url} - triggering auto-logout`,
                );
